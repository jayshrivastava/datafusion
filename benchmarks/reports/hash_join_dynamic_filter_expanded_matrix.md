...
Typically, each `filter_expr_for_partition` takes the form of a range expression plus a set membership expression, for example:

```
(some_min <= expr AND expr <= some_max) AND expr IN (some_set_of_keys)
```

Per-partition `CASE` routing was added in https://github.com/apache/datafusion/pull/18451, but there were no benchmarks.

The `CASE` expression has one tradeoff:
...
This benchmark compares 4 alternatives:
...
Typically, each `filter_expr_for_partition` takes the form of a range expression plus a set membership expression, like `(some_min <= expr AND expr <= some_max) AND expr IN (some_set_of_keys)`.
...
- idea: let the cheap bounds expression evaluate first before evaluating the expensive `CASE` expression
...
First, when is turning dynamic filtering off good?

All wins for `A off` are when
(a) pruning is disabled (`row_filter_only`) or
(b) clustering is low (Q24 and Q25), meaning row group pruning is not effective.

This is backed by metrics in the appendix below. For Q24 and Q25, no row groups are pruned:

```
row_groups_pruned_statistics = 524 total -> 524 matched
```

For Q26 and Q27, far more row groups are pruned:
...
Let's focus on cases where dynamic filtering is useful. These are scores when `pruning=true`:
...
`global` and `bounds+case` are effectively equivalent in terms of performance. If Parquet pruning is the most
important type of filtering, the `CASE` part of `bounds+case` is likely less useful. `global`
also has an edge in simplicity.
