...
`global` and `bounds+case` are effectively equivalent in terms of performance.

`global` likely has a slight edge if we consider these:
- since parquet pruning is important, the `case` part of `bounds+case` is likely less useful than the bounds because it does not get pushed down
- `global` filters are more simple to generate. This would make distributed dynamic filtering (for comet, ballista, distributed datafusion) a bit easier
  to implement. Notably, dynamic filters in trino and spark does not have per-shuffle-partition special casing:
  https://github.com/datafusion-contrib/datafusion-distributed/blob/js/dynamic-filtering-design/rfcs/1-distributed-dynamic-filtering.md
